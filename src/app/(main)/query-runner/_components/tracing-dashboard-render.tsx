...
export default function QueryDashboard({ tracingInfo }: { tracingInfo: TracingResult }) {
  const [data, setData] = useState<TracingResult>(tracingInfo)

  // State variables for various datasets
  const [shardData, setShardData] = useState<{ name: string; events: number; totalTime: number }[]>([])
  ...
  useEffect(() => {
    ...
    const activityDurationPerShard = new Map<string, { [activity: string]: number }>()
    ...
    data.events.forEach((event, index) => {
      ...
      shardActivities[event.activity] = (shardActivities[event.activity] || 0) + duration

      // Events per Thread
      eventsPerThread.set(event.thread, (eventsPerThread.get(event.thread) || 0) + 1)

      // Activity Frequency
      activityFrequency.set(event.activity, (activityFrequency.get(event.activity) || 0) + 1)
      ...
      activitiesPerSource.set(event.source, (activitiesPerSource.get(event.source) || 0) + 1)
    ...
    // Shard Data
    const shardDataArray = Array.from(shardMap.entries()).map(([name, info]) => ({
      name,
      events: info.events,
    ...
    const eventsPerThreadArray = Array.from(eventsPerThread.entries()).map(([thread, count]) => ({
      thread,
      count,
    }))
    ...
    const activityFrequencyArray = Array.from(activityFrequency.entries()).map(([activity, count]) => ({
      activity,
      count,
    }))
    ...
    const activitiesPerSourceArray = Array.from(activitiesPerSource.entries()).map(([source, count]) => ({
      source,
      count,
    }))
  ...
  const colors = ["#8884d8", "#82ca9d", "#ffc658", "#d0ed57", "#a4de6c", "#8dd1e1", "#83a6ed", "#8e4585"]
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p><strong>Coordinator:</strong> {data.coordinator}</p>
            <p><strong>Command:</strong> {data.command}</p>
            <p><strong>Request:</strong> {data.request}</p>
            <p><strong>Duration:</strong> {data.duration} ms</p>
            <p><strong>Total Source Elapsed:</strong> {totalSourceElapsed} ms</p>
            <p><strong>Started at:</strong> {new Date(data.started_at).toLocaleString()}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p><strong>Consistency Level:</strong> {data.parameters.consistency_level}</p>
            <p><strong>Serial Consistency Level:</strong> {data.parameters.serial_consistency_level}</p>
            <p><strong>Param[0]:</strong> {data.parameters['param[0]'] || 'N/A'}</p>
            <p><strong>Query:</strong> {data.parameters.query}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <ScrollArea className="h-[400px]">
            {data.events.map((event) => (
              <div key={event.event_id} className="mb-2 p-2 rounded">
                ...
                <p><strong>Thread:</strong> {event.thread}</p>
                <p><strong>Source:</strong> {event.source}</p>
                <p><strong>Elapsed Time:</strong> {event.source_elapsed} ms</p>
              </div>
            ))}
          </ScrollArea>
        </CardContent>
