...
import { Breadcrumb, BreadcrumbItem, BreadcrumbLink } from "@scylla-studio/components/ui/breadcrumb"
            ...
            <Card className="mb-4">
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <strong>Replication Strategy:</strong> NetworkTopologyStrategy
                  </div>
                  <div>
            ...
            <Card>
              ...
              <CardContent>
                <ul className="space-y-2">
                  ...
                  <li className="flex items-center">
                    <Eye className="mr-2 h-4 w-4" /> daily_sales (Materialized View)
                  </li>
                </ul>
              </CardContent>
            </Card>
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100">
        ...
        <header className="bg-white shadow-sm">
          <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
            <h1 className="text-2xl font-semibold text-gray-900">ScyllaDB Dashboard</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <Breadcrumb>
            <BreadcrumbItem>
              <BreadcrumbLink href="#">{selectedConnection || "Select Connection"}</BreadcrumbLink>
            </BreadcrumbItem>
            {activeTab.includes('-') && (
              <BreadcrumbItem>
                <BreadcrumbLink href="#">{activeTab.split('-')[0]}</BreadcrumbLink>
              </BreadcrumbItem>
            )}
            {/* <BreadcrumbItem isCurrentPage> */}
            <BreadcrumbItem >
              <BreadcrumbLink href="#">{activeTab.includes('-') ? activeTab.split('-')[1] : activeTab}</BreadcrumbLink>
            </BreadcrumbItem>
          </Breadcrumb>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <ScrollArea className="w-full whitespace-nowrap">
              <TabsList>
                <TabsTrigger value="manage-connections" className="flex items-center">
                  Manage Connections
                  <Button variant="ghost" size="sm" className="ml-2 p-0" onClick={() => setActiveTab("")}>
                    <X className="h-4 w-4" />
                  </Button>
                </TabsTrigger>
                ...
                <TabsTrigger value="ecommerce-products" className="flex items-center">
                  [T] products
                  <Button variant="ghost" size="sm" className="ml-2 p-0" onClick={() => setActiveTab("")}>
                    <X className="h-4 w-4" />
                  </Button>
                </TabsTrigger>
                <TabsTrigger value="ecommerce-daily_sales" className="flex items-center">
                  [MV] daily_sales
                  <Button variant="ghost" size="sm" className="ml-2 p-0" onClick={() => setActiveTab("")}>
                    <X className="h-4 w-4" />
                  </Button>
                </TabsTrigger>
                <TabsTrigger value="ecommerce-address" className="flex items-center">
                  [UDT] address
                  <Button variant="ghost" size="sm" className="ml-2 p-0" onClick={() => setActiveTab("")}>
                    <X className="h-4 w-4" />
                  </Button>
