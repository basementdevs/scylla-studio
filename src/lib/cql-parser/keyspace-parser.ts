...
export function parseCreateKeyspace(row: DescriptionRow): KeyspaceDefinition {
    ...
    if (withClauseMatch) {
        ...
        const durableWritesMatch = withClause.match(/durable_writes\s*=\s*(true|false)/i);
        if (durableWritesMatch) {
            durableWrites = durableWritesMatch[1].toLowerCase() === 'true';
        }

        // Extract tablets
        const tabletsMatch = withClause.match(/tablets\s*=\s*\{.*'enabled'\s*:\s*(true|false)\}/i);
        if (tabletsMatch) {
...
function parseReplication(withClause: string, datacenters: Map<string, number>) {
    const replicationMatch = withClause.match(/replication\s*=\s*\{(.+?)\}/);
    if (replicationMatch) {
        const replicationSettings = replicationMatch[1].split(', ');
        replicationSettings.forEach(setting => {
            const [key, value] = setting.split(':').map(part => part.trim().replace(/['"]/g, ''));

            if (key !== 'class') {
                // Assume it's a datacenter replication factor
                datacenters.set(key, parseInt(value, 10));
            }
        });
    }
}
