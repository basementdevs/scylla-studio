...
export function addConnection(connection: Connection) {
  const {name, host, port, status, username, password, dc, nodes} = connection;
...
export function updateConnectionById(
  ...
  const {name, host, username, status, password, dc, nodes, port} = updatedConnection;
  const stmt = db.prepare(`
    UPDATE connections
    SET name = ?, host = ?, username = ?, status = ?, password = ?, dc = ?, nodes = ?, port = ?
    WHERE id = ?
  `);
  stmt.run(name, host, username, status, password, dc, nodes, port, connectionId);
}
